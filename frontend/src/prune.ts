...
(function () {
	...
	let iterator; // either forEachEnumerableOwnProperty, forEachEnumerableProperty or forEachProperty

	// iterates on enumerable own properties (default behavior)
	const forEachEnumerableOwnProperty = function(obj, callback) {
		for (const k in obj) {
			if (Object.prototype.hasOwnProperty.call(obj, k)) callback(k);
		}
	};
	// iterates on enumerable properties
	const forEachEnumerableProperty = function(obj, callback) {
		for (const k in obj) callback(k);
	};
	// iterates on properties, even non enumerable and inherited ones
	// This is dangerous
	const forEachProperty = function (obj, callback, excluded) {
		if (obj == null) return;
		excluded = excluded || {};
	...
	function str(key, holder, depthDecr, arrayMaxLength) {
		let i, k, v, length, partial, value = holder[key];
